...
const STATUS_CLASS = {
  ...
  "Submitted on portal (self-reported)":"status-submitted",
  "Under Review":"status-review", "Interview Scheduled":"status-interview",
  "Rejected":"status-rejected", "Selected":"status-selected"
};
// Illustrative-only progression for platform ("internal") applications, since this
// demo has no real employer/ATS integration. Deterministic per job so it's stable
// across refreshes, not randomised each time.
function simulatedStatus(app) {
  if (app.jobApplyType !== "internal") return app.status;
  const days = daysUntil(app.appliedDate) * -1;
  let h = 0; for (const c of app.jobId) h = (h * 31 + c.charCodeAt(0)) | 0;
  const outcome = Math.abs(h) % 3;
  if (days < 2) return "Submitted";
  if (days < 5) return "Under Review";
  if (days < 8) return "Interview Scheduled";
  return outcome === 0 ? "Selected" : outcome === 1 ? "Rejected" : "Interview Scheduled";
}
function pageApplications() {
  ...
  <div class="page container">
    ...
    <div class="card card-pad" style="margin-bottom:16px">
      <p class="small muted" style="margin:0">Platform ("One-Click") applications below show an illustrative status timeline since this demo isn't connected to a real employer ATS — plug in an API integration to replace it with live statuses. External-portal applications only change status when you tell us you've completed them there.</p>
    </div>
    <div class="app-list">
      ${apps.length ? apps.map(a => {
        const job = JOBS.find(j => j.id === a.jobId);
        if (!job) return "";
        const status = simulatedStatus(a);
        const showSelfReport = a.jobApplyType === "external" && a.status === "Redirected — complete on portal";
        return `
        <div class="app-row">
          <div>
          ...
          <div><span class="status-badge ${STATUS_CLASS[status]||"status-review"}">${escapeHtml(status)}</span></div>
          <div class="flex gap-8">
            <a href="#/job/${job.id}" class="btn btn-outline btn-sm">View job</a>
            ${showSelfReport ? `<button class="btn btn-primary btn-sm" data-self-report="${job.id}">Mark submitted</button>` : ""}
          </div>
        </div>`;
...
function bindPageEvents() {
  ...
  const selfReportBtns = qsa("[data-self-report]");
  selfReportBtns.forEach(b => b.addEventListener("click", () => {
    const app = state.applications.find(a => a.jobId === b.dataset.selfReport);
    if (app) { app.status = "Submitted on portal (self-reported)"; app.lastUpdated = new Date().toISOString().slice(0,10); persist(); render(); toast("Marked as submitted — thanks for the update", "✅"); }
  }));
}
...
function handleApply(jobId) {
  ...
  if (job.applyType === "internal") {
    ...
    state.applications.push({ jobId, status: "Submitted", appliedDate: today, jobApplyType: "internal", lastUpdated: today });
    persist();
    ...
    el("confirmExternalApply").addEventListener("click", () => {
      ...
      const today = new Date().toISOString().slice(0,10);
      state.applications.push({ jobId, status: "Redirected — complete on portal", appliedDate: today, jobApplyType: "external", lastUpdated: today });
      persist();
      closeModal();
      toast("Redirected to official portal — finish your application there, then mark it done from your tracker", "↗️");
      render();
    });
  }
}
