...
// actually used at the time of applying, not a canned template. Downloads as a
// standalone HTML file the candidate can open, print to PDF, or attach manually
// to a portal that doesn't accept auto-fill.
function buildTailoredDocHtml(snapshot, job) {
  ...
  const extra = pSkills.filter(s => !matched.map(m=>m.toLowerCase()).includes(s.toLowerCase()));
  const esc = escapeHtml;
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${esc(snapshot.name)} — ${esc(job.title)}</title>
  <style>body{font-family:Arial,Helvetica,sans-serif;max-width:680px;margin:40px auto;color:#111;line-height:1.5}
  h1{font-size:22px;margin-bottom:2px} h2{font-size:14px;color:#1D4ED8;border-bottom:2px solid #1D4ED8;padding-bottom:4px;margin-top:26px}
  .muted{color:#555} .tag{display:inline-block;background:#E8EEFE;color:#1D4ED8;padding:3px 9px;border-radius:12px;font-size:12px;margin:2px 4px 2px 0}
  .tag.extra{background:#F1F1F1;color:#555}</style></head><body>
  <h1>${esc(snapshot.name || "Candidate")}</h1>
  <p class="muted">${esc(snapshot.email||"")}${snapshot.phone ? " · "+esc(snapshot.phone) : ""}${snapshot.city ? " · "+esc(snapshot.city) : ""}</p>
  <h2>Applying for</h2>
  <p><strong>${esc(job.title)}</strong> at ${esc(job.company)} (${esc(job.city)})</p>
  <h2>Why this profile fits this role</h2>
  <p>${esc(snapshot.name||"This candidate")} has ${esc(String(snapshot.experienceYears ?? 0))} year(s) of experience and a ${esc(snapshot.education||"relevant")} background, matching ${matched.length} of ${jSkills.length} core skills requested for this role.</p>
  <h2>Matching skills for this role</h2>
  <p>${matched.length ? matched.map(s=>`<span class="tag">${esc(s)}</span>`).join("") : '<span class="muted">No direct skill overlap on file — consider adding relevant skills to your profile.</span>'}</p>
  ${extra.length ? `<h2>Additional skills</h2><p>${extra.map(s=>`<span class="tag extra">${esc(s)}</span>`).join("")}</p>` : ""}
  <h2>Education &amp; experience</h2>
  <p>${esc(snapshot.education||"—")} · ${esc(String(snapshot.experienceYears ?? 0))} years of experience</p>
  <h2>Resume on file</h2>
  <p class="muted">${snapshot.resumeName ? "📎 " + esc(snapshot.resumeName) + " (attach this file alongside this sheet)" : "No resume file was on file at the time of this application."}</p>
  <p class="muted" style="margin-top:36px;font-size:11px">Auto-generated by Job in One Click on ${esc(new Date().toISOString().slice(0,10))} from the candidate's profile at the time of application. Not a substitute for the candidate's full resume.</p>
  </body></html>`;
}
function downloadTailoredDoc(jobId) {
  ...
  const html = buildTailoredDocHtml(snapshot, job);
  const blob = new Blob([html], { type: "text/html" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = `${slug(snapshot.name)}_${slug(job.company)}_${slug(job.title)}_tailored.html`;
  document.body.appendChild(a); a.click(); a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 2000);
  toast("Tailored highlights downloaded — open it, or print to PDF to attach", "📄");
}
...
function pageApplications() {
  ...
  <div class="page container">
    ...
    <div class="card card-pad" style="margin-bottom:16px">
      <p class="small muted" style="margin:0"><strong>What's automated vs. what's yours to confirm:</strong> resume selection, tailored highlight sheets, and prefill are fully automatic per role. Status itself is self-tracked — no outside app can honestly pull "real" updates from an employer's private hiring system without that employer's API access, so update it yourself as things happen (a call, an email, a portal update) and it saves instantly and shows everywhere right away. See <a href="#/settings">Settings → Integrations</a> for how a real backend would plug in live sync.</p>
    </div>
    <div class="card card-pad" style="margin-bottom:16px">
      <p class="small muted" style="margin:0">📄 Every application below auto-generated a <strong>role-tailored highlights sheet</strong> from your profile at the moment you applied — download it anytime from the row below.</p>
    </div>
    <div class="app-list">
      ${apps.length ? apps.map(a => {
        ...
        <div class="app-row">
          ...
          <div class="flex gap-8" style="flex-wrap:wrap">
            ...
            <button class="btn btn-outline btn-sm" data-tailor="${job.id}">📄 Tailored sheet</button>
...
function bindGlobalActions() {
  document.body.addEventListener("click", (e) => {
    ...
    if (tailorBtn) { downloadTailoredDoc(tailorBtn.dataset.tailor); return; }
  });
...
function handleApply(jobId) {
  ...
  if (job.applyType === "internal") {
    ...
    state.applications.push({ jobId, status: "Submitted", appliedDate: today, jobApplyType: "internal", lastUpdated: today, resumeUsed: resumeChoice.fileName, resumeTag: resumeChoice.tag, history: [{status:"Submitted", date: today}], profileSnapshot: snapshot });
    persist();
    downloadTailoredDoc(jobId);
    toast(`Applied with your "${resumeChoice.tag}" resume ✅ — tailored highlights generated`, "✅");
    render();
  } else {
    showModal(`
    ...
    el("confirmExternalApply").addEventListener("click", () => {
      ...
      const today = new Date().toISOString().slice(0,10);
      const snapshot = { ...state.profile };
      state.applications.push({ jobId, status: "Redirected — complete on portal", appliedDate: today, jobApplyType: "external", lastUpdated: today, resumeUsed: resumeChoice.fileName, resumeTag: resumeChoice.tag, history: [{status:"Redirected — complete on portal", date: today}], profileSnapshot: snapshot });
      persist();
      downloadTailoredDoc(jobId);
      closeModal();
      toast("Redirected to official portal — tailored highlights downloaded to attach there", "↗️");
      render();
    });
  }
}
