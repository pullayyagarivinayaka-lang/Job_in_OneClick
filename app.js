...
function slug(s) { return String(s||"").toLowerCase().replace(/[^a-z0-9]+/g,"_").replace(/^_+|_+$/g,"") || "candidate"; }

// Real, per-job resume tailoring — computed client-side from the profile snapshot
// actually used at the time of applying. Rendered directly inside the platform
// (a modal), never downloaded as a separate file.
function fitVerdict(score) {
  if (score === null) return { label: "Complete your profile for a compatibility read", cls: "match-low" };
  if (score >= 70) return { label: "Strong fit — covers most of what this role asks for", cls: "match-high" };
  if (score >= 45) return { label: "Partial fit — some requirements met, some gaps below", cls: "match-mid" };
  return { label: "Limited fit — this role asks for skills/experience not yet on your profile", cls: "match-low" };
}
function buildTailoredResumeInner(snapshot, job, resumeUsed) {
  const esc = escapeHtml;
  const pSkills = snapshot.skills || [];
  const jSkills = job.skills || [];
  const matched = jSkills.filter(s => pSkills.map(x=>x.toLowerCase()).includes(s.toLowerCase()));
  const missing = jSkills.filter(s => !matched.map(m=>m.toLowerCase()).includes(s.toLowerCase()));
  const fitScore = matchScore(job, snapshot);
  const verdict = fitVerdict(fitScore);
  return `
    <h3 style="margin-bottom:2px">${esc(snapshot.name || "Candidate")}</h3>
    <p class="muted small" style="margin:0 0 14px">${esc(snapshot.email||"")}${snapshot.phone ? " · "+esc(snapshot.phone) : ""}${snapshot.city ? " · "+esc(snapshot.city) : ""}</p>
    <div class="card card-pad" style="margin-bottom:14px">
      <div class="flex justify-between items-center" style="flex-wrap:wrap;gap:8px">
        <strong>Compatibility for this role</strong>
        ${fitScore !== null ? `<span class="match-badge ${matchClass(fitScore)}">${fitScore}% fit</span>` : ""}
      </div>
      <p class="small muted" style="margin:8px 0 0">${esc(verdict.label)}</p>
    </div>
    <h4>Applying for</h4>
    <p style="margin:0 0 12px"><strong>${esc(job.title)}</strong> — ${esc(job.company)}, ${esc(job.city)}</p>
    <h4>Skills highlighted to match this role's requirements</h4>
    <div class="tag-row" style="margin-bottom:10px">${matched.length ? matched.map(s=>`<span class="tag blue">${esc(s)}</span>`).join("") : `<span class="small muted">No direct overlap found on your profile.</span>`}</div>
    ${missing.length ? `<h4>Requirements this role lists that aren't on your profile yet</h4><div class="tag-row" style="margin-bottom:10px">${missing.map(s=>`<span class="tag">${esc(s)}</span>`).join("")}</div>` : ""}
    <h4>Education &amp; experience presented for this role</h4>
    <p class="small muted" style="margin:0 0 10px">${esc(snapshot.education||"—")} · ${esc(String(snapshot.experienceYears ?? 0))} years experience</p>
    <h4>Resume used for this application</h4>
    <p class="small muted" style="margin:0">📎 ${esc(resumeUsed || snapshot.resumeName || "No resume on file")}</p>
    <p class="small muted" style="margin-top:18px;padding-top:14px;border-top:1px solid var(--border)">This is Job in One Click's own automated read of your profile against this role's stated requirements — generated the moment you applied, kept with this application, and not the employer's actual hiring decision.</p>
  `;
}
function showTailoredResume(jobId) {
  const app = state.applications.find(a => a.jobId === jobId);
  const job = JOBS.find(j => j.id === jobId);
  if (!job) return;
  const snapshot = app?.profileSnapshot || state.profile;
  if (!snapshot) { toast("No profile data available to build this from", "⚠️"); return; }
  const inner = buildTailoredResumeInner(snapshot, job, app?.resumeUsed);
  showModal(`<div style="max-height:65vh;overflow-y:auto;padding-right:4px">${inner}</div><div class="modal-actions"><button class="btn btn-primary" data-close-modal>Close</button></div>`);
}

/* ---------------- Automated screening evaluation ---------------- */
// Runs entirely from data already on the job + the profile snapshot used at
// apply time — deterministic, synchronous, no network calls, so it can never
// silently fail or go stale. This is Job in One Click's own fit read, not a
// signal from the employer's real screening process.
function evaluateFit(job, snapshot) {
  const score = matchScore(job, snapshot);
  const v = fitVerdict(score);
  const jSkills = job.skills || [];
  const pSkills = (snapshot?.skills || []).map(s => s.toLowerCase());
  const missing = jSkills.filter(s => !pSkills.includes(s.toLowerCase()));
  return { score, label: v.label, cls: v.cls, missing };
}
...
function profileCompleteness() {
  ...
  if (!p) return 0;
  const hasResume = p.resumeName || (p.resumes||[]).length > 0;
  const fields = [p.name, p.email, p.phone, p.education, p.experienceYears !== undefined, (p.skills||[]).length, hasResume, (p.preferredCategories||[]).length];
  const filled = fields.filter(Boolean).length;
  return Math.round((filled / fields.length) * 100);
}
// Autonomous resume selection: picks whichever tailored resume shares the most
// categories with the job; falls back to the general resume; returns null if
// the candidate has uploaded nothing at all.
function pickResumeForJob(job, profile) {
  if (!profile) return null;
  const resumes = profile.resumes || [];
  let best = null, bestScore = 0;
  resumes.forEach(r => {
    const score = job.categories.includes(r.category) ? 1 : 0;
    if (score > bestScore) { bestScore = score; best = r; }
  });
  if (best) return { fileName: best.fileName, tag: best.category };
  if (profile.resumeName) return { fileName: profile.resumeName, tag: "General" };
  return null;
}
        ...
        <div class="card card-pad">
          ...
          <p class="small muted mt-16" style="margin:0">
            ${job.applyType === "internal"
              ? "This employer accepts applications directly through Job in One Click. We automatically attach whichever resume in your profile best matches this role's categories — no manual selection needed. The lookup link above is optional."
              : "This employer's application is hosted on an external portal. \"Apply Now\" opens it with your details prefilled where supported and tells you which resume to attach; the direct link above skips prefill if you'd rather fill it in yourself. Either way, you'll complete and submit it on their site — not here."}
          </p>
        </div>
        ${breakdown ? `
        <div class="card card-pad mt-16">
...
function pageApplications() {
  ...
  <div class="page container">
    ...
    <div class="card card-pad" style="margin-bottom:16px">
      <p class="small muted" style="margin:0"><strong>What's automated vs. what's yours to confirm:</strong> resume selection, the tailored resume view, and the fit screening below all run instantly and automatically, right here in the platform. The employer's actual hiring status is the one thing no outside app can honestly know without that employer's API access — so that part is self-tracked: update it yourself as things happen and it saves instantly everywhere. See <a href="#/settings">Settings → Integrations</a> for how a real backend would plug in live employer sync.</p>
    </div>
    <div class="app-list">
      ${apps.length ? apps.map(a => {
        ...
        const screening = a.screening || evaluateFit(job, a.profileSnapshot || state.profile || {});
        return `
        <div class="app-row">
          <div>
            ...
            <div class="mt-8">
              <span class="match-badge ${screening.cls}">${screening.score ?? "—"}% automated fit</span>
              ${screening.missing.length ? `<div class="small muted mt-8">Gaps: ${screening.missing.slice(0,3).map(escapeHtml).join(", ")}${screening.missing.length>3?"…":""}</div>` : `<div class="small muted mt-8">All listed skills matched</div>`}
            </div>
          </div>
          <div class="small"><span class="muted">Applied</span><br>${fmtDate(a.appliedDate)}</div>
          <div class="small"><span class="muted">Mode</span><br>${a.jobApplyType === "internal" ? "One-Click" : "External portal"}${a.resumeUsed ? `<br><span class="muted">📎 ${escapeHtml(a.resumeTag)} resume</span>` : ""}</div>
          <div>
          ...
          <div class="flex gap-8" style="flex-wrap:wrap">
            ...
            <button class="btn btn-outline btn-sm" data-tailor="${job.id}">📄 View tailored resume</button>
...
function pageProfile() {
  ...
  <div class="page container">
    ...
    <form id="profileForm" class="card card-pad">
      ...
      <div class="field mt-16">
        <label>General resume (fallback)</label>
        <div class="upload-box ${p.resumeName ? "has-file" : ""}" id="uploadBox">
          ...
          <p class="small" style="margin:0">${p.resumeName ? "📎 " + escapeHtml(p.resumeName) : "No file uploaded yet. Used automatically when no tailored version below matches."}</p>
        </div>
      </div>
      <div class="field mt-16">
        <div class="flex justify-between items-center">
          <label style="margin:0">Tailored resumes by role (optional, recommended)</label>
        </div>
        <p class="hint" style="margin-top:0">Upload a different resume per role type. When you apply, the app automatically picks whichever one best matches that job — no manual choice needed.</p>
        <div id="resumeLibraryList" class="grid" style="gap:10px">
          ${(p.resumes||[]).map((r,i) => `
            <div class="card card-pad flex justify-between items-center" style="flex-wrap:wrap;gap:10px">
              <div>
                <strong class="small">${escapeHtml(r.category)}</strong>
                <div class="muted small">📎 ${escapeHtml(r.fileName)}</div>
              </div>
              <button type="button" class="btn btn-outline btn-sm" data-remove-resume="${i}">Remove</button>
            </div>`).join("")}
        </div>
        <div class="flex gap-8 mt-8" style="flex-wrap:wrap;align-items:center">
          <select class="input" id="newResumeCategory" style="max-width:220px">
            ${CATEGORIES.map(c => `<option>${c}</option>`).join("")}
          </select>
          <input type="file" id="newResumeFile" accept=".pdf,.doc,.docx" class="sr-only">
          <label for="newResumeFile" class="btn btn-outline btn-sm" style="cursor:pointer">+ Add tailored resume</label>
        </div>
      </div>
...
function pageSettings() {
  ...
  <div class="page container" style="max-width:640px">
    ...
    <div class="card card-pad mt-16">
      <div><strong>Integrations</strong> <span class="tag" style="background:var(--surface-2);color:var(--text-muted)">Not connected — needs a backend</span></div>
      <p class="muted small" style="margin:8px 0 12px">This build has no server, so there's nothing to authenticate against yet. In production, live status sync would come from whichever of these an employer actually grants access to — not from this app reaching into their systems on its own:</p>
      <ul class="muted small" style="margin:0 0 0 18px;padding:0">
        <li>ATS webhooks (Greenhouse, Lever, Workday) — employer pushes status changes to your backend</li>
        <li>Job board "Easy Apply" APIs (LinkedIn, Indeed) — official, sanctioned auto-apply for participating employers only</li>
        <li>Email parsing (with explicit user consent) to detect status-change emails automatically</li>
      </ul>
    </div>
    <div class="card card-pad mt-16">
      <div class="flex justify-between items-center">
...
function bindGlobalActions() {
  document.body.addEventListener("click", (e) => {
    ...
    const tailorBtn = e.target.closest("[data-tailor]");
    if (tailorBtn) { showTailoredResume(tailorBtn.dataset.tailor); return; }
  });
...
function bindPageEvents() {
  ...
  if (pf) {
    ...
    el("resumeFile").addEventListener("change", (e) => {
      ...
      if (f) { state.profile.resumeName = f.name; persist(); toast("General resume attached: " + f.name, "📎"); render(); }
    });
    const newResumeFile = el("newResumeFile");
    if (newResumeFile) newResumeFile.addEventListener("change", (e) => {
      const f = e.target.files[0];
      const category = el("newResumeCategory").value;
      if (f) {
        state.profile.resumes = state.profile.resumes || [];
        state.profile.resumes = state.profile.resumes.filter(r => r.category !== category);
        state.profile.resumes.push({ category, fileName: f.name });
        persist();
        toast(`Tailored resume added for "${category}"`, "📎");
        render();
      }
    });
    qsa("[data-remove-resume]").forEach(b => b.addEventListener("click", () => {
      const idx = Number(b.dataset.removeResume);
      state.profile.resumes = (state.profile.resumes || []).filter((_, i) => i !== idx);
      persist();
      render();
    }));
  }
...
function handleApply(jobId) {
  ...
  const completion = profileCompleteness();
  const resumeChoice = pickResumeForJob(job, state.profile);
  if (!resumeChoice || completion < 60) {
    showModal(`
      <h3>Complete your profile first</h3>
      <p class="muted">One-Click Apply auto-selects a resume from your profile for each role. Please add your details and upload at least one resume before applying.</p>
  ...
  if (job.applyType === "internal") {
    ...
    const screening = evaluateFit(job, snapshot);
    state.applications.push({ jobId, status: "Submitted", appliedDate: today, jobApplyType: "internal", lastUpdated: today, resumeUsed: resumeChoice.fileName, resumeTag: resumeChoice.tag, history: [{status:"Submitted", date: today}], profileSnapshot: snapshot, screening });
    persist();
    toast(`Applied with your "${resumeChoice.tag}" resume ✅ — automated screening: ${screening.score}% fit`, "✅");
    render();
  } else {
    showModal(`
      <h3>You'll apply on ${escapeHtml(job.company)}'s official portal</h3>
      <p class="muted">Job in One Click doesn't submit this application for you. We'll open the official portal in a new tab and prefill your details where the portal supports it — you'll need to review, attach your <strong>${escapeHtml(resumeChoice.tag)}</strong> resume (${escapeHtml(resumeChoice.fileName)}), and submit it there.</p>
      <div class="modal-actions">
        <button class="btn btn-outline" data-close-modal>Cancel</button>
    ...
    el("confirmExternalApply").addEventListener("click", () => {
      ...
      const snapshot = { ...state.profile };
      const screening = evaluateFit(job, snapshot);
      state.applications.push({ jobId, status: "Redirected — complete on portal", appliedDate: today, jobApplyType: "external", lastUpdated: today, resumeUsed: resumeChoice.fileName, resumeTag: resumeChoice.tag, history: [{status:"Redirected — complete on portal", date: today}], profileSnapshot: snapshot, screening });
      persist();
      closeModal();
      toast(`Redirected to official portal — automated screening: ${screening.score}% fit`, "↗️");
      render();
    });
  }
}
